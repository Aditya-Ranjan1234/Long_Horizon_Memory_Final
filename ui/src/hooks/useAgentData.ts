...
export const useAgentData = () => {
  ...
  const buildStateSignature = (state: AgentState) =>
    [
      ...
      state.correct_in_memory.toString(),
      state.total_relevant_seen.toString(),
    ].join('|');
  ...
  const normalizeStateEvent = (raw: any): AgentState | null => {
    ...
    return {
      ...
      task_name: String(observation?.task_name ?? payload?.task_name ?? observation?.metadata?.task ?? 'easy'),
      correct_in_memory: Number(observation?.metadata?.correct_in_memory ?? payload?.correct_in_memory ?? 0),
      total_relevant_seen: Number(observation?.metadata?.total_relevant_seen ?? payload?.total_relevant_seen ?? 0),
      memory_capacity: Number(observation?.metadata?.memory_capacity ?? payload?.memory_capacity ?? 8),
    ...
    const playbackTimer = window.setInterval(() => {
      ...
      setLogs((prev) => {
        const nextLog: TrainingLog = {
          ...
          fmt_reward: renderedState.task_score,
